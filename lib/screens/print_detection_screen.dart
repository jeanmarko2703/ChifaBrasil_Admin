...
class PrintDetectionScreen extends StatefulWidget {
   
  const PrintDetectionScreen({Key? key}) : super(key: key);

  @override
  State<PrintDetectionScreen> createState() => _PrintDetectionScreenState();
}

class _PrintDetectionScreenState extends State<PrintDetectionScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final printerListProvider= Provider.of<PrinterListProvider>(context);
    
   
    // print(!printerListProvider.printers.isEmpty?'':printerListProvider.printers[0]);
    return  Scaffold(
      appBar: AppBar(automaticallyImplyLeading: true, title: Text('Impresoras'), ),
      body: _devices.isEmpty?Center(child: Text(_devicesMsg),) :ListView.builder(
        itemCount: _devices.length,
        itemBuilder: (context, i){
          return ListTile( onTap: (){

            showDialog(context: context, builder: (context)=>AlertDialog(title: Text('Quieres configurar esta impresora?') ,actions: [
              ...
              ElevatedButton(onPressed: (){
                ...
                printerListProvider.newPrinter(_devices[i].address.toString());
                 print('el scanner actual es:');
            ...
            // _startPrint(_devices[i], orderFinal);
